 ...
 * keeps and maintains the list of turtles. Implements executor, which is an interface.
 * 
 * This class demonstrates reflection, which is used to call the turtle/pen methods. The
 * CommandUnit object returns a Commandable method, which can then be called by this class.
 * This ensures encapsulation, as the executor class does not have to see the methods or know which
 * methods correspond with which user-entered commands in order to control the turtle and pen objects.
 * This class also maintains the main observable, which allows for all other observables to remain 
 * observing the same object, even as active turtles change. This class also demonstrates the use of
 * the channel, which ensures that the executor and the interpreter can communicate without the need
 * for either section to see instances of the other's classes.
 * 
...
public class SlogoExecutor implements Executor {
	private static final String CHANGE_TURTLES = "tell";
	...
	private void setup(){
		...
		newTurtle(1); //creates the first turtle
	...
	private double[] calc(CommandUnit c){
		...
		if (c.getCommand().equals(CHANGE_TURTLES)) {
			result = new double[1];
